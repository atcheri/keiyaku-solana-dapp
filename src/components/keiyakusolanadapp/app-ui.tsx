...
import { useKeiyakuProgram, useKeiyakuProgramAccount } from "./app-data-access";
import { useWallet } from "@solana/wallet-adapter-react";

export function KeiyakuCreate() {
  const { createVestingAccount } = useKeiyakuProgram();
...
export const KeiyakuList = () => {
  const { accounts, getProgramAccount } = useKeiyakuProgram();
...
function KeiyakuCard({ account }: { account: PublicKey }) {
  const { accountQuery, createEmployeeVestingAccount } =
    useKeiyakuProgramAccount({
      account,
    });
